...
export default function ContratCard({ 
  ...
  // Calculer le montant total perçu
  const totalPerçu = contrat.paiements
    ?.filter((p: any) => p.statut === 'EFFECTUE')
    .reduce((sum: number, p: any) => sum + p.montant, 0) || 0;

  // Vérifier les impayés
  const impayés = contrat.paiements
...
if (isCompact) {
  // Version compacte pour la liste dans la modale
  return (
    <motion.div 
  ...
  return (
    ...
    >
      ...
      <div className="contrat-card-body">
        ...
        {totalPerçu > 0 && (
          <div className="contrat-total-percu">
            <span className="total-label">Total perçu</span>
            <span className="total-valeur">{formatMoney(totalPerçu)}</span>
          </div>
        )}

        {impayés > 0 && (
          <div className="contrat-impaye-banner">
            <span className="banner-icon">⚠️</span>
            <span className="banner-text">{impayés} paiement(s) en retard</span>
          </div>
        )}
      ...
      <div className="contrat-card-footer">
        ...
        {onAddPaiement && isActif && (
          <button 
            className="action-btn payment"
            onClick={() => onAddPaiement(contrat)}
