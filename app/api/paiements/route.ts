...
export async function PUT(
  ...
  try {
    ...
    const paiementExistants = await queryRows(
      'SELECT contrat_id, type_transaction FROM paiements WHERE id = ?',
      [id]
    ) as any[];
    ...
    const isVente = paiementExistants[0].type_transaction === 'VENTE';
    const montantSaisi = parseFloat(montant);
    ...
    if (isVente && type_vente) {
      const totalVerse = await queryRows(
        `SELECT SUM(montant) as total FROM paiements 
         WHERE contrat_id = ? AND id != ? AND type_transaction = 'VENTE'`,
        [contrat_id, id]
      ) as any[];
      ...
      if (type_vente === 'ACOMPTE' && montantSaisi > prixVente) {
        return NextResponse.json(
          { success: false, erreur: "L'acompte ne peut pas dépasser le prix total" },
          { status: 400 }
        );
      ...
      if (type_vente === 'VERSEMENT' && nouveauTotal > prixVente) {
        return NextResponse.json(
          { success: false, erreur: 'Le total des versements dépasse le prix de vente' },
          { status: 400 }
        );
      ...
      if (type_vente === 'SOLDE' && Math.abs(nouveauTotal - prixVente) > 1) {
        return NextResponse.json(
          { success: false, erreur: 'Le solde doit correspondre au prix total' },
          { status: 400 }
        );
